...
def run(args, log):
    ...
    for issue in issue_jsons:
        ...
        for comment in issue["comments"]:
            comments.append(
                {
                    "body": f"**Comment from {format_user(comment['user'])} at "
                    f"{format_comment_time(issue, comment)}**\n\n{comment['comment']})"
                }
            )
        comments_params = {"comments": comments}
        if g.rate_limit.core.remaining < 100:
            log.info(
                f"Rate has reached a limit - {g.rate_limit.core.remaining}. Sleeping for 61 minutes"
            )
            time.sleep(60 * 61)
